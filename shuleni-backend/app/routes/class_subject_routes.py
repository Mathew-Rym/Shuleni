...
class_subject_bp = Blueprint('class_subjects', __name__)

class_subject_bp.route('/<int:class_id>/subjects', methods=['POST'])(
    jwt_required()(roles_required('admin', 'educator')(school_required(assign_subject_to_class)))
)
...
class_subject_bp.route('/<int:class_id>/subjects/<int:subject_id>', methods=['DELETE'])(
    jwt_required()(roles_required('admin', 'educator')(school_required(remove_subject_from_class)))
)
